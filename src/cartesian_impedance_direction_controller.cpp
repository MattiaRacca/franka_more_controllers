...
#include <franka_more_controllers/cartesian_impedance_direction_controller.h>
...
bool CartesianImpedanceDirectionController::init(hardware_interface::RobotHW* robot_hw,
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "CartesianImpedanceDirectionController: Invalid or no joint_names parameters provided, "
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceDirectionController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceDirectionController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceDirectionController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceDirectionController: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceDirectionController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "CartesianImpedanceDirectionController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
...
void CartesianImpedanceDirectionController::starting(const ros::Time& /*time*/) {
...
void CartesianImpedanceDirectionController::update(const ros::Time& /*time*/,
...
PLUGINLIB_EXPORT_CLASS(franka_more_controllers::CartesianImpedanceDirectionController,
