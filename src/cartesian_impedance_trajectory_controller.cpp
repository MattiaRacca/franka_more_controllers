...
bool CartesianImpedanceTrajectoryController::init(hardware_interface::RobotHW* robot_hw,
  ...
  speed_target_ = 0.0;
  rotation_speed_ = 0.0;
  rotation_speed_target_ = 0.0;
  distance_to_cover_ = 0.0;

  motion_vector_.setZero();
...
void CartesianImpedanceTrajectoryController::update(const ros::Time& /*time*/,
  ...
  // TODO: it will never stop... it has no concept of target reached
  // TODO: if (current-desired < 0.01) then commanded = desired else commanded = current + velocity
  // Something like that?
  position_d_ += motion_vector_*(speed_*period.toSec());
  ...
  error.tail(3) << error_quaternion_angle_axis.axis() * (error_quaternion_angle_axis.angle()*(rotation_speed_*period.toSec()));
  ...
  /* NOT SURE I NEED THE FILTER HERE!
...
void CartesianImpedanceTrajectoryController::targetPoseCallback(const geometry_msgs::PoseStampedConstPtr& msg)
{
  ...
  // direction of motion
  motion_vector_ = position_d_target_ - position_d_;

  // distance to
  distance_to_cover_ = motion_vector_.norm();

  motion_vector_.normalize();

  double desired_time = distance_to_cover_/speed_target_;

  // change in orientation
  // TODO: is the orientation_d_ updated in the main loop? Maybe change to current_orientation?
  Eigen::Quaterniond error_quaternion(orientation_d_ * last_orientation_d_target.inverse());
  // convert to axis angle
  Eigen::AngleAxisd error_quaternion_angle_axis(error_quaternion);

  // Based on the positional speed, change the rotation speed
  rotation_speed_target_ = error_quaternion_angle_axis.angle()/desired_time;
}
...
void CartesianImpedanceTrajectoryController::update(const ros::Time& /*time*/,
  ...
  // add velocity component to the desired position
  // TODO: it will never stop... it has no concept of target reached
  // TODO: if (current-desired < threshold) then commanded = desired else commanded = current + velocity
  // Something like that? or maybe the motion_vector and friends need to be updated?
  // OR threat the motion_vector_ as not normalized!! and, keeping track of the passed time, you should have the velocity
